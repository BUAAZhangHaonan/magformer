...
def calculate_rgb_stats_for_dir(rgb_dir):
    ...
    try:
        image_files = [f for f in os.listdir(
            rgb_dir) if f.endswith(('.png', '.jpg', '.jpeg'))]
        if not image_files:
...
def main(args):
    ...
    for subset in subsets:
        ...
        if mean is not None and std is not None:
            ...
            print("MODEL:")
            print("  PIXEL_MEAN: [{:.4f}, {:.4f}, {:.4f}]".format(
                mean[0], mean[1], mean[2]))
            print("  PIXEL_STD: [{:.4f}, {:.4f}, {:.4f}]".format(
                std[0], std[1], std[2]))
            print("-" * 40)
...
if __name__ == "__main__":
    ...
    # 输入参数变为数据集根目录
    parser.add_argument('--dataset_root', type=str, default='/home/remote1/zhanghaonan/projects/mask2former_datasets/0909_512_0.12K',
