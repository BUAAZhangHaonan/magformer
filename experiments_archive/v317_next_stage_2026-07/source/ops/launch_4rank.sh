#!/bin/bash
# launch_4rank.sh — launch one 4-rank training arm on GPUs 4-7 (physical, no
# CUDA_VISIBLE_DEVICES remap: runtime.gpus addresses cuda:4-7 directly)
# + start the host-memory watchdog on its process group.
# Usage: launch_4rank.sh <config-name> [master_port]
set -eu
CFG="${1:?usage: launch_4rank.sh <config-name> [port]}"
PORT="${2:-29611}"
...
sleep 2
setsid bash "$SRC/ops/watch_host_memory.sh" "$PID" "$CFG" >/dev/null 2>&1 &
