...
def load_finetune_weights(
    ...
    from magformer.engine.utils import (
    ...
    if bool(getattr(model, "residual_depth_fusion_enabled", False)):
        ...
        for projection in model.rgb_backbone.depth_projections.values():
            if torch.count_nonzero(projection.weight).item() != 0:
                raise RuntimeError(
                    "Residual depth fusion warm-start changed a "
                    "zero-initialized projection weight"
                )
            if projection.bias is not None and torch.count_nonzero(
                projection.bias
            ).item() != 0:
                raise RuntimeError(
                    "Residual depth fusion warm-start changed a "
                    "zero-initialized projection bias"
                )
