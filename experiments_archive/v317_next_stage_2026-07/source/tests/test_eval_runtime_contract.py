...
def test_trainer_evaluate_uses_inference_contract_and_logs_metrics_only(
    ...
    monkeypatch.setattr(
        ...
        lambda metrics, **kwargs: best_artifact_calls.append(
                dict(metrics, _artifact_kind=kwargs.get("artifact_kind", "model_best"))
            ),
    )
    ...
    # First eval improves both the canonical-AP best and the AP_s best.
    assert len(best_artifact_calls) == 2
    kinds = {c["_artifact_kind"] for c in best_artifact_calls}
    assert kinds == {"model_best", "model_best_aps"}
    for call in best_artifact_calls:
        assert call["val/mAP"] == metrics["val/mAP"]
...
def test_single_gpu_and_ddp_evaluation_return_the_same_metrics(
    ...
    ddp_trainer.best_aps_metric = float("-inf")
    trainer.best_aps_metric = float("-inf")
