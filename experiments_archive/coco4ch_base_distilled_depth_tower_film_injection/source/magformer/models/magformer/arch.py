...
class MagFormerArch(nn.Module):
    ...
    def __init__(
        ...
        point_refiner: Optional[nn.Module] = None,
        num_classes: int = 1,
        num_queries: int = 100,
        hidden_dim: int = 256,
        ...
        self.point_refiner = point_refiner
        self.point_refiner_enabled = point_refiner is not None
    ...
    def from_config(cls, config: Any) -> "MagFormerArch":
        ...
        if depth_only:
            ...
            swin_backend = getattr(model_cfg.swin, "backend", "d2")
            _rgb_name = str(model_cfg.rgb_backbone.name).lower()
            if "mobilenetv3" in _rgb_name:
                # MobileNetV3 RGB backbone (timm ImageNet init, 3-channel).
                _mbv3_variant = "large" if "large" in _rgb_name else "small"
                from ..common.backbones.mobilenet import MobileNetV3Depth
                rgb_backbone = MobileNetV3Depth(
                    variant=_mbv3_variant,
                    out_features=list(model_cfg.swin.out_features),
                    pretrained=model_cfg.rgb_backbone.pretrained,
                    weights_path=model_cfg.rgb_backbone.weights,
                    in_chans=3,
                )
                use_rgb_pretrained = False
            elif swin_backend == "d2":
                rgb_backbone = D2SwinBackbone(
                    ...
                    in_chans=int(getattr(model_cfg.swin, "in_chans", 3)),
                )
            else:
        ...
        point_refiner_cfg = model_cfg.point_refiner
        point_refiner = None
        if point_refiner_cfg.enabled:
            point_refiner = RGBDPointRefiner(
                train_num_points=point_refiner_cfg.train_num_points,
                subdivision_steps=point_refiner_cfg.subdivision_steps,
                subdivision_num_points=point_refiner_cfg.subdivision_num_points,
            )
        ...
        model.rgb_concat_depth = bool(
            getattr(model_cfg, "rgb_concat_depth", False))
        model.depth_tower_inject = [
            int(s) for s in getattr(model_cfg, "depth_tower_inject", []) or []]
        if model.depth_tower_inject:
            from .depth_tower import DepthTower, FiLM
            stage_dims = [
                int(rgb_backbone._stage_out_channels["res{}".format(i + 2)])
                for i in range(4)
            ]
            model.depth_tower = DepthTower(in_chans=1)
            model.depth_films = nn.ModuleList(
                [FiLM(c, d) for c, d in zip(DepthTower.CHANS, stage_dims)])
            print(
                "[CDTI] depth tower injection enabled at stages "
                f"{model.depth_tower_inject}, dims={stage_dims}")
    ...
    def _sync_criterion_from_config(self, config: Any) -> None:
        ...
        bbox_w = float(getattr(mask_former, "bbox_loss_weight", 5.0))
        giou_w = float(getattr(mask_former, "giou_loss_weight", 2.0))
        weight_dict["loss_bbox"] = bbox_w
        weight_dict["loss_giou"] = giou_w
        if getattr(mask_former, "deep_supervision", False):
            num_aux = max(int(mask_former.dec_layers) - 1, 0)
            for i in range(num_aux):
                weight_dict[f"loss_bbox_{i}"] = bbox_w
                weight_dict[f"loss_giou_{i}"] = giou_w
        ...
        self.criterion = SetCriterion(
            ...
            eos_coef=eos_coef,
            losses=("labels", "masks") + (("boxes",) if (bbox_w > 0 or giou_w > 0) else ()),
    ...
    def _build_point_detail_features(
        self,
        images: torch.Tensor,
        depths: torch.Tensor,
        depth_valid_masks: Optional[torch.Tensor],
    ) -> Optional[torch.Tensor]:
        if self.point_refiner is None:
            return None
        if depth_valid_masks is None:
            raise ValueError("RGB-D point refinement requires depth_valid_masks")
        normalized_rgb = (images - self.pixel_mean) / self.pixel_std
        return self.point_refiner.build_detail_features(
            normalized_rgb, depths, depth_valid_masks
        )
    ...
    def _extract_dual_features(
        ...
        if getattr(self, "rgb_concat_depth", False):
            # RGB-D 4-channel concat input (detectron2 M2F rgbd_concat parity):
            # dataset depth is [0,1] float; scale to [0,255] and append as 4th
            # channel before 4-element pixel_mean/std normalization.
            images_in = torch.cat(
                [images, depths.to(images.dtype) * 255.0], dim=1)
            images_norm = (images_in - self.pixel_mean) / self.pixel_std
            rgb_features = self.rgb_backbone(images_norm)
            inject_stages = getattr(self, "depth_tower_inject", None)
            if inject_stages:
                from .depth_tower import apply_film_injection
                rgb_features = apply_film_injection(
                    rgb_features,
                    self.depth_tower,
                    self.depth_films,
                    inject_stages,
    ...
    def forward(
        ...
        point_detail_features = (
            self._build_point_detail_features(images, depths, depth_valid_masks)
            if self.training
            else None
        )
        ...
        if self.training:
            ...
            losses = self.criterion(
                outputs,
                processed_targets,
                detail_features=point_detail_features,
                point_refiner=self.point_refiner,
            )
            if fusion_losses:
    ...
    def forward_inference_decoder_outputs(
        ...
        point_detail_features = self._build_point_detail_features(
            images, depths, depth_valid_masks
        )
        ...
        if point_detail_features is not None:
            outputs["point_detail_features"] = point_detail_features
        return outputs
    ...
    def _inference_raw(
        ...
        class_scores = pred_logits.softmax(-1)[..., :-1]
        ...
        if self.point_refiner is not None:
            detail_features = outputs.get("point_detail_features")
            if detail_features is None:
                raise RuntimeError(
                    "point_refiner inference requires point_detail_features"
                )
            masks = self.point_refiner.subdivide(masks, detail_features)
