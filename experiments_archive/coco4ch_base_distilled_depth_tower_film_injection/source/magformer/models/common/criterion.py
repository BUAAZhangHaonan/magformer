...
class SetCriterion(nn.Module):
    ...
    def forward(
        ...
        detail_features: Optional[torch.Tensor] = None,
        point_refiner: Optional[RGBDPointRefiner] = None,
        ...
        refinement_enabled = point_refiner is not None
        if refinement_enabled and detail_features is None:
            raise ValueError("point_refiner requires detail_features")
        ...
        for loss_name in self.losses:
            losses.update(self._get_loss(
                ...
                detail_features=detail_features if loss_name == "masks" else None,
                point_refiner=point_refiner if loss_name == "masks" else None,
            ))
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                if refinement_enabled:
                    aux_indices = indices
                    aux_scale_weights = scale_weights
                else:
    ...
    def _loss_masks(
        ...
        with torch.no_grad():
            point_coords = get_uncertain_point_coords_with_randomness(
                src_masks,
                num_points=(
                    point_refiner.train_num_points
        ...
        coarse_point_logits = point_sample(src_masks, point_coords)
        if point_refiner is None:
            point_logits = coarse_point_logits.squeeze(1)
        else:
            point_logits = point_refiner(
                coarse_point_logits,
                detail_features,
                point_coords,
                src_idx[0],
