...
class CopyPasteConfig(BaseModel):
    """Copy-Paste augmentation configuration for small object oversampling."""

    enabled: bool = Field(default=False, description="Enable copy-paste augmentation")
    prob: float = Field(default=0.5, description="Probability of applying copy-paste per image")
    max_paste_instances: int = Field(default=5, description="Max instances to paste per image")
    min_instance_area: int = Field(default=16, description="Skip instances smaller than this (pixels)")
    max_instance_area_ratio: float = Field(default=0.3, description="Skip instances larger than this fraction of image area")
    prefer_small: bool = Field(default=True, description="Bias selection toward objects < 32x32 pixels")
    small_threshold: int = Field(default=1024, description="Area threshold for 'small' objects (default 32x32=1024 px)")
...
class DataConfig(BaseModel):
    ...
    copy_paste: CopyPasteConfig = Field(
        default_factory=CopyPasteConfig, description="Copy-Paste augmentation configuration"
    )

    # SAHI-style crop training (small tile crop for small object AP boost)
    sahi_crop_size: Optional[int] = Field(
        default=None,
        description="SAHI crop size for training. When set (e.g. 768), crops to a smaller tile instead of image_size, making small objects occupy a larger fraction. None = disabled (backward compatible)."
    )
...
class ModalityFusionConfig(BaseModel):
    ...
    residual_alpha: float = Field(default=0.0, description="保留字段；当前实现要求为 0")
    ...
    dccg_conf_hidden: int = Field(
        default=16, gt=0, description="DCCG depth confidence estimator hidden channels")
    dccg_use_confidence: bool = Field(
        default=True, description="Whether DCCG uses learned depth confidence gating")
    model_config = ConfigDict(extra="allow")
...
class MaskFormerConfig(BaseModel):
    ...
    cost_bbox: float = Field(default=0.0, ge=0.0, description="Hungarian 匹配的 cxcywh L1 box cost")
    cost_giou: float = Field(default=0.0, ge=0.0, description="Hungarian 匹配的 GIoU box cost")
    ...
    num_feature_levels: int = Field(default=3, description="Number of feature levels for transformer decoder")
    # Deformable cross-attention for decoder
    use_deformable_cross_attn: bool = Field(default=False, description="Replace dense cross-attention with multi-scale deformable attention in the transformer decoder. More memory-efficient, better for small objects.")
    deformable_n_points: int = Field(default=4, description="Number of sampling points per head per level in deformable cross-attention")
    ...
    co_detr_enabled: bool = Field(
        default=False, description="Enable Co-DETR auxiliary FCOS heads on pixel decoder features. Training-only, zero inference cost.")
    co_detr_loss_weight: float = Field(
        default=1.0, description="Loss weight multiplier for Co-DETR auxiliary losses (focal + GIoU + centerness)")
    ...
    @classmethod
    def normalize_legacy_train_num_points(cls, values):
        if not isinstance(values, dict) or "train_num_points" not in values:
            return values

        normalized = dict(values)
        legacy_num_points = normalized["train_num_points"]
        warnings.warn(
            "model.magformer.mask_former.train_num_points is deprecated; use "
            "matcher_num_points and loss_num_points. The legacy value is applied "
            "only to split fields that are not explicitly set.",
            FutureWarning,
            stacklevel=3,
        )
        normalized.setdefault("matcher_num_points", legacy_num_points)
        normalized.setdefault("loss_num_points", legacy_num_points)
        return normalized
...
class ResidualDepthFusionConfig(BaseModel):
    """Stage-entry residual depth injection configuration."""

    enabled: bool = Field(default=False)
    encoder: Literal["light_depth_pyramid", "mobilenetv3_large"] = Field(
        default="light_depth_pyramid"
    )

    model_config = ConfigDict(extra="forbid")


class PointRefinerConfig(BaseModel):
    """Query-preserving RGB-D point-mask refinement."""

    enabled: bool = Field(default=False)
    train_num_points: int = Field(default=2048, gt=0)
    subdivision_steps: Literal[2] = Field(default=2)
    subdivision_num_points: int = Field(default=4096, gt=0)
...
class FiLMConfig(BaseModel):
    """FiLM modulation layer configuration."""

    enabled: bool = Field(default=False, description="Whether to enable FiLM modulation")
    location: str = Field(default="pixel_decoder", description="Where to apply FiLM: 'pixel_decoder', 'decoder', 'fusion'")
    hidden_dim: int = Field(default=256, description="FiLM hidden layer dimension")

    model_config = ConfigDict(extra="allow")


class GeometryAttentionConfig(BaseModel):
    """Depth geometry attention bias configuration."""

    enabled: bool = Field(default=False, description="Whether to enable geometry attention bias")
    decay_range: tuple = Field(default=(0.75, 1.0), description="Decay range for multi-head beta parameters")
    mode: str = Field(default="decomposed", description="Bias mode: 'decomposed' for row/col decomposition")

    model_config = ConfigDict(extra="allow")


class DepthEdgeAttnConfig(BaseModel):
    """Depth-edge guided attention configuration."""

    enabled: bool = Field(default=False, description="Whether to enable depth-edge guided attention")
...
class MagFormerModelConfig(BaseModel):
    ...
    depth_only: bool = Field(
        default=False,
        description="Ablation: 完全去掉 RGB backbone 和 fusion，depth_backbone 直连 pixel_decoder。"
                    "启用时忽略 modality_fusion.enabled 和 rgb_backbone。",
    )
    ...
    residual_depth_fusion: ResidualDepthFusionConfig = Field(
        default_factory=ResidualDepthFusionConfig,
        description="Stage-entry residual depth injection configuration",
    )

    point_refiner: PointRefinerConfig = Field(
        default_factory=PointRefinerConfig,
        description="Query-preserving RGB-D point-mask refinement",
    )

    # FiLM modulation
    film: FiLMConfig = Field(
        default_factory=FiLMConfig, description="FiLM modulation configuration"
    )

    # Geometry attention bias
    geometry_attn: GeometryAttentionConfig = Field(
        default_factory=GeometryAttentionConfig, description="Depth geometry attention bias configuration"
    )

    # Depth-edge attention
    depth_edge_attn: DepthEdgeAttnConfig = Field(
        default_factory=DepthEdgeAttnConfig, description="Depth-edge guided attention configuration"
    )
...
class ModelConfig(BaseModel):
    ...
    small_object_sample_threshold: int = Field(
        default=0,
        ge=0,
        description=(
            "Retired criterion sampling knob. Only 0 (disabled) is supported; "
            "model construction rejects non-zero values."
        ),
    )
...
class SolverConfig(BaseModel):
    ...
    iteration_unit: Literal["optimizer_step", "legacy_micro_step"] = Field(
        default="optimizer_step",
        description=(
            "Unit for max_iter, warmup_iters, scheduler milestones, and runtime "
            "periods. optimizer_step counts successful parameter updates; "
            "legacy_micro_step preserves the historical microbatch contract."
        ),
    )
    warmup_factor: float = Field(default=1.0e-3, description="预热因子")
    ...
    clip_value: float = Field(default=1.0, description="裁剪值")
...
class EarlyStopConfig(BaseModel):
    """Explicit, opt-in early-stopping policy for maximized validation metrics."""

    enabled: bool = Field(
        default=False, description="Whether early stopping is enabled"
    )
    monitor: str = Field(
        default="val/mAP", description="Validation metric to maximize"
    )
    patience: int = Field(
        default=5,
        ge=1,
        description="Eligible plateau evaluations before stopping",
    )
    min_delta: float = Field(
        default=0.001,
        ge=0.0,
        allow_inf_nan=False,
        description="Minimum significant metric improvement",
    )
    target: Optional[float] = Field(
        default=None,
        allow_inf_nan=False,
        description="Optional metric target that stops training",
    )
    min_optimizer_step: int = Field(
        default=0,
        ge=0,
        description="Ignore early-stop observations before this optimizer step",
    )

    @model_validator(mode="before")
    @classmethod
    def normalize_legacy_target_ap(cls, values):
        if not isinstance(values, dict) or "target_ap" not in values:
            return values

        normalized = dict(values)
        if "target" in normalized:
            raise ValueError(
                "runtime.early_stop cannot define both deprecated target_ap and target"
            )
        warnings.warn(
            "runtime.early_stop.target_ap is deprecated; use target. Early "
            "stopping is opt-in and remains disabled unless enabled: true is set.",
            FutureWarning,
            stacklevel=3,
        )
        normalized["target"] = normalized.pop("target_ap")
        return normalized
...
class RuntimeConfig(BaseModel):
    ...
    memory_fraction: Optional[float] = Field(
        ...
        le=1.0,
        allow_inf_nan=False,
        description="Optional per-process PyTorch CUDA allocator fraction",
    )
    ...
    cpu_threads: int = Field(
        default=8,
        ge=1,
        description="PyTorch intra-op and native math-library thread cap",
    )
    cpu_interop_threads: int = Field(
        default=1,
        ge=1,
        description="PyTorch inter-op thread cap",
    )
    ...
    amp_init_scale: float = Field(
        ...
        gt=0.0,
        allow_inf_nan=False,
        description="GradScaler 初始缩放值",
    )
    max_consecutive_amp_skips: int = Field(
        default=16,
        ge=0,
        description="连续 AMP overflow 跳步上限；超过后终止训练",
    )
    early_stop: EarlyStopConfig = Field(
        default_factory=EarlyStopConfig,
        description="Explicit opt-in early stopping configuration",
    )
    eval_iou_types: Optional[List[str]] = Field(default=None, description="COCO 评估 IoU 类型")
    eval_batch_size: int = Field(default=1, description="eval dataloader batch size")
    eval_max_images: Optional[int] = Field(default=None, description="最多评估图像数")
    eval_max_dets: int = Field(default=100, description="COCO eval maxDets 参数")
