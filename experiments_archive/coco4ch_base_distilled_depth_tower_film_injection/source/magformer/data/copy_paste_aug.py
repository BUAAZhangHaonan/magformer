...
class CopyPasteAugmentation:
    ...
    def _paste_instance(
        ...
        if scale != 1.0:
            ...
            source_crop_img = cv2.resize(
                source_crop_img, (new_w, new_h), interpolation=cv2.INTER_LINEAR
            )
    ...
    def __call__(
        ...
        for src_image, src_mask, src_bbox, src_cat in selected:
            ...
            image, mask_canvas, success = self._paste_instance(
                image, mask_canvas, instance_id, crop_img, crop_mask, paste_y, paste_x, scale
            )
