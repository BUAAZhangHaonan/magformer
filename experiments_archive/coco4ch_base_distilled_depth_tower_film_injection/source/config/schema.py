...
class MaskFormerConfig(BaseModel):
    ...
    mask_loss_upsample: bool = Field(default=False, description="Upsample pred masks to full resolution before loss computation")
    ...
    mask_feature_upsample: bool = Field(
        default=False, description="Learned PixelShuffle upsampler for mask_features (stride-4 -> stride-2). Zero-init so starting point = bilinear upsampling.")
