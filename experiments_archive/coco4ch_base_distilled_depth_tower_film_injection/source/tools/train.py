...
def _extract_model_state_dict(checkpoint_obj: Any) -> Dict[str, Any]:
    """Require the current model-artifact checkpoint contract."""
    if not isinstance(checkpoint_obj, dict):
        raise TypeError(
            "Finetune checkpoint must be a dictionary containing model_state_dict"
        )
    state_dict = checkpoint_obj.get("model_state_dict")
    if not isinstance(state_dict, dict):
        raise TypeError(
            "Finetune checkpoint must contain a dictionary model_state_dict"
        )
    if not state_dict or not all(isinstance(key, str) for key in state_dict):
        raise ValueError("model_state_dict must contain string-keyed model tensors")
    return state_dict


_RESIDUAL_PROJECTION_KEYS = {
    f"rgb_backbone.depth_projections.{stage}.{suffix}"
    for stage in ("res2", "res3", "res4", "res5")
    for suffix in ("weight", "bias")
}


def _prepare_finetune_state_dict(
    model: torch.nn.Module,
    checkpoint_state: Dict[str, Any],
) -> Dict[str, Any]:
    """Apply the one current residual-depth checkpoint namespace conversion."""
    model_state = model.state_dict()
    residual_enabled = bool(
        getattr(model, "residual_depth_fusion_enabled", False)
    )
    ...
    for source_key, value in checkpoint_state.items():
        ...
        if residual_enabled and source_key.startswith("depth_backbone."):
            target_key = (
                "rgb_backbone.depth_encoder."
                + source_key[len("depth_backbone.") :]
            )
            mapped_keys.append({"source": source_key, "target": target_key})

        if target_key in converted:
            raise RuntimeError(
                "Finetune checkpoint namespace conversion collided at "
                f"{target_key!r}"
            )

        if (
            residual_enabled
            and target_key not in model_state
            and source_key.startswith(("fusion.", "pixel_decoder.depth_pe."))
        ):
            dropped_keys.append(source_key)
            continue
        converted[target_key] = value

    unexpected_keys = sorted(key for key in converted if key not in model_state)
    shape_mismatches = []
    for key in sorted(set(converted) & set(model_state)):
        checkpoint_value = converted[key]
    ...
    missing_keys = sorted(key for key in model_state if key not in converted)
    allowed_missing = set()
    if bool(getattr(model, "point_refiner_enabled", False)):
        allowed_missing.update(
            key for key in model_state if key.startswith("point_refiner.")
        )
    if residual_enabled:
        actual_projection_keys = {
            key
            for key in model_state
            if key.startswith("rgb_backbone.depth_projections.")
        }
        if actual_projection_keys != _RESIDUAL_PROJECTION_KEYS:
            raise RuntimeError(
                "Residual-depth model projection tensors do not match the current "
                f"architecture: {sorted(actual_projection_keys)}"
            )
        allowed_missing.update(_RESIDUAL_PROJECTION_KEYS)

    invalid_missing = sorted(set(missing_keys) - allowed_missing)
    invalid_allowed = sorted(allowed_missing - set(missing_keys))
    if unexpected_keys or shape_mismatches or invalid_missing or invalid_allowed:
        raise RuntimeError(
            "Finetune checkpoint is incompatible with the current model: "
            f"unexpected={unexpected_keys}, shape_mismatches={shape_mismatches}, "
            f"invalid_missing={invalid_missing}, expected_missing_but_present={invalid_allowed}"
        )
...
def load_finetune_weights(
    ...
    Moves the model to CPU before validating and loading model tensors.

    Returns:
        Exact matched, mapped, dropped, missing, unexpected, and shape audit.
    """
    ...
    compatibility = _prepare_finetune_state_dict(model, checkpoint_state)
    incompatible = model.load_state_dict(compatibility["state_dict"], strict=False)
    loaded_missing = sorted(incompatible.missing_keys)
    loaded_unexpected = sorted(incompatible.unexpected_keys)
    if loaded_missing != compatibility["missing_keys"] or loaded_unexpected:
        raise RuntimeError(
            "Validated finetune state changed during model loading: "
            f"missing={loaded_missing}, unexpected={loaded_unexpected}"
        )

    print(f"[Train] Warm-start loaded model weights from {filepath}")
    print(
        "[Train] Warm-start compatibility: "
        f"checkpoint={compatibility['checkpoint_key_count']}, "
        f"model={compatibility['model_key_count']}, "
        f"mapped={len(compatibility['mapped_keys'])}, "
        f"dropped={len(compatibility['dropped_keys'])}, "
        f"matched={len(compatibility['matched_keys'])}, "
        f"missing={len(compatibility['missing_keys'])}, unexpected=0, "
        "shape_mismatches=0"
    )
    if compatibility["missing_keys"]:
        print(f"[Train] Approved missing keys: {compatibility['missing_keys']}")

    # Move model back to original device after loading
    model.to(original_device)

    compatibility.pop("state_dict")
    compatibility["checkpoint_path"] = str(filepath)
...
def main():
    ...
    if init_mode == "finetune":
        ...
        load_finetune_weights(model, finetune_path)
