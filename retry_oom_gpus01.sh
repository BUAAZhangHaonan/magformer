...
# Retry OOM models from full19 run at 1024 with 2 GPUs (0,1 DDP)
...
run_model() {
  local model_id="$1"
  shift
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] [retry-oom-01] Starting: ${model_id}"

  # Clean staging dir to avoid stale checkpoints
  rm -rf "${STAGING}/${model_id}"

  if "$@"; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [retry-oom-01] DONE: ${model_id}"
  else
    local rc=$?
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [retry-oom-01] FAILED rc=${rc}: ${model_id}"
  fi
}

echo "[$(date '+%Y-%m-%d %H:%M:%S')] [retry-oom-01] Starting OOM retry with CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
echo "[$(date '+%Y-%m-%d %H:%M:%S')] [retry-oom-01] EVAL_PERIOD_OVERRIDE=${EVAL_PERIOD_OVERRIDE}"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] [retry-oom-01] All OOM model retries completed"

# 6. Stardist (single GPU, uses stardist conda env with TF)
# cuDNN fix already applied: nvidia-cudnn-cu12 installed in stardist env
echo "[$(date '+%Y-%m-%d %H:%M:%S')] [retry-oom-01] Starting stardist retry (TF GPU fix)"
...
CUDA_VISIBLE_DEVICES=0 \
  bash "${SCRIPTS}/run_0831_1k_20ep_1024_revisit_stardist_inst.sh" \
    ...
    --run \
  && echo "[$(date '+%Y-%m-%d %H:%M:%S')] [retry-oom-01] DONE: stardist" \
  || echo "[$(date '+%Y-%m-%d %H:%M:%S')] [retry-oom-01] FAILED: stardist"

echo "[$(date '+%Y-%m-%d %H:%M:%S')] [retry-oom-01] All retries (OOM + stardist) completed"
