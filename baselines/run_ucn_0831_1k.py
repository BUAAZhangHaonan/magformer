#!/usr/bin/env python3
"""
Run UCN (Unseen Object Clustering Network) baseline on ECC 0831_1K (RGBD) and
evaluate with COCOeval (segm/bbox AP).
...
from run_ucn_ecc import build_ucn_recipe


def _workspace_root() -> Path:
    # File: <ws>/magformer/baselines/run_ucn_0831_1k.py
...
class ECC0831UCNDataset(torch.utils.data.Dataset):
    """
    ...
    def __init__(self, dataset_root: str, split: str, img_size: int, train: bool, pixel_mean_bgr_255: List[float]):
...
def main() -> None:
    ...
    args = ap.parse_args()
    recipe = build_ucn_recipe("0831")

    if args.dataset_root is None:
        args.dataset_root = str(_workspace_root() / "magformer_datasets" / "0831_1K")
    ...
    cfg.PIXEL_MEANS = np.array([[[28.1363, 30.5413, 34.9731]]], dtype=np.float32)

    train_ds = ECC0831UCNDataset(
    ...
    print(f"[ucn] train={len(train_ds)} val={len(val_ds)} epochs={cfg.epochs} batch={args.batch} img={args.img_size}")
