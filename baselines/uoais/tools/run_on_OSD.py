...
if __name__ == "__main__":
    ...
    for rgb_path, depth_path in zip(rgb_paths, depth_paths):
        ...
        # ------------------- 修改后的代码 (替换上面的代码) -------------------
        # 1. 定义一个输出目录，我们使用项目根目录下的 'output' 文件夹
        output_dir = "output"

        # 2. 从输入文件的完整路径中，只提取出文件名 (e.g., "000000_color.png")
        base_filename = os.path.basename(rgb_path)

        # 3. 构造一个有意义的输出文件名 (e.g., "000000_color_result.png")
        save_filename = os.path.splitext(base_filename)[0] + "_result.png"

        # 4. 拼接成完整的保存路径
        save_path = os.path.join(output_dir, save_filename)

        # 5. 使用 cv2.imwrite 保存最终的可视化图像
        cv2.imwrite(save_path, vis_all_img)

        # 6. (可选) 打印一条信息，告诉我们文件已保存
        print(f"Result for {base_filename} saved to {save_path}")
