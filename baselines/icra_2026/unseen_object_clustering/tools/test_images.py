...
def save_data(file_rgb, out_label_refined, roi, features_crop):
    ...
    output_root_dir = '/home/remote1/zhanghaonan/projects/icra_2026_experiment/UnseenObjectClustering/output'
    os.makedirs(output_root_dir, exist_ok=True)
    base_filename = os.path.basename(file_rgb)
    output_filename = base_filename.replace('-color.png', '-label.png')
    filename = os.path.join(output_root_dir, output_filename)
    # meta data
    '''
    ...
    for i in index_images:
        if os.path.exists(images_color[i]):
            ...
            if out_label_refined is not None:
                save_data(images_color[i], out_label_refined, None, None)
        else:
