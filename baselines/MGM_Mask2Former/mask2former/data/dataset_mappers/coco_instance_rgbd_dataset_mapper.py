...
from ..depth_utils import normalize_and_augment_depth
...
def _normalize_and_augment_depth(depth: np.ndarray, cfg) -> np.ndarray:
    return normalize_and_augment_depth(depth, cfg)
...
def _read_noise_mask_any(path: str) -> np.ndarray:
    ...
    if ext in [".npy", ".npz"]:
        ...
        if arr.ndim == 3:
            ...
            raise ValueError(
                f"Unexpected noise mask shape in npy: {arr.shape}")
...
def _rgb_to_depth_npy_path(rgb_abs_path: str, dataset_root: str) -> str:
    """Depth npy path lookup with backward-compatible roots.

    Preferred:
      DATASET_ROOT/depth/depth_npy/<split>/<same_name>.npy(.npz)
    Fallback:
      DATASET_ROOT/depth/<split>/<same_name>.npy(.npz)
    """
    rel = _relpath_under_images(
        rgb_abs_path, dataset_root)  # e.g. 'train/.../xxx.png'
    rel_no_ext = os.path.splitext(rel)[0]
    depth_roots = [
        os.path.join(dataset_root, "depth", "depth_npy"),
        os.path.join(dataset_root, "depth"),
    ]
    tried = []
    for root in depth_roots:
        cand = os.path.join(root, rel_no_ext + ".npy")
        if os.path.isfile(cand):
            return cand
        cand_npz = os.path.splitext(cand)[0] + ".npz"
        if os.path.isfile(cand_npz):
            return cand_npz
        tried.extend([cand, cand_npz])
...
def _rgb_to_noise_mask_path(rgb_abs_path: str, dataset_root: str) -> str:
    ...
    for e in exts:
        cand = os.path.join(dataset_root, "depth",
                            "depth_noise_mask", rel_no_ext + e)
        if os.path.isfile(cand):
            return cand
    raise FileNotFoundError("")
...
class COCOInstanceRGBDDatasetMapper:
    ...
    def __init__(
        ...
        *,
        ...
        if not self.dataset_root or not os.path.isdir(self.dataset_root):
            raise ValueError(
                f"INPUT.DATASET_ROOT is invalid: {self.dataset_root}")
        ...
        if nm_cfg is not None:
            ...
            self.noise_mask_check_dir = bool(
                getattr(nm_cfg, "CHECK_DIR", True))
        ...
        candidate_depth_dirs = [
            os.path.join(self.dataset_root, "depth", "depth_npy"),
            os.path.join(self.dataset_root, "depth"),
        ]
        for p in [os.path.join(self.dataset_root, "images")]:
            if not os.path.isdir(p):
                logging.getLogger(__name__).warning(
                    f"[Mapper] Missing directory: {p}")
        if not any(os.path.isdir(p) for p in candidate_depth_dirs):
            logging.getLogger(__name__).warning(
                f"[Mapper] Missing depth directories: {candidate_depth_dirs}")
    ...
    def __call__(self, dataset_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # --- 读取 Depth (NPY/NPZ) ---
        # 由 register_coco_instances 组成的绝对路径
        ...
        if self.noise_mask_enabled and self.noise_mask_available:
            try:
                ...
                noise_arr = _read_noise_mask_any(
                    nm_path)  # HxWx1, float32 in {0,1}
        ...
        content_mask = transforms.apply_segmentation(
            np.ones((orig_h, orig_w), dtype=np.uint8)).astype(bool)
