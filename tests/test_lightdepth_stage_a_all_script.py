...
def test_lightdepth_stage_a_all_lists_expected_candidates() -> None:
    ...
    assert "mobilenetv3_directadd_edge" in text
    assert "mobilenetv3_gatedadd_edge" in text
    assert "resnet18_gatedadd_edge" in text
    assert "mobilenetv3_film_edge_validhole" in text
