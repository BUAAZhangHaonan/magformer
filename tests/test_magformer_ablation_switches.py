...
class _PixelDecoder(torch.nn.Module):
    def forward(self, features, confidence_maps=None, depth_raw=None, padding_mask=None):
        # Pass-through minimal contract used by MagFormerArch
        mem = features["res2"]
        return {"memory": mem, "mask_features": mem}


class _TransformerDecoder(torch.nn.Module):
    def forward(self, memory, mask_features, multi_scale_features=None, multi_scale_pos=None, pos_key=None):
        b = memory.shape[0]
