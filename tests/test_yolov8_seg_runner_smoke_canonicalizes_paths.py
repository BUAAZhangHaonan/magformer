...
def test_yolov8_seg_trackp_smoke_canonicalizes_out_dir_and_project(tmp_path: Path) -> None:
    ...
    out_dir = (tmp_path / out_root / "yolov8_seg").as_posix()

    assert f"--out-dir '{out_dir}'" in res.stdout
    assert f"project='{out_dir}'" in res.stdout
