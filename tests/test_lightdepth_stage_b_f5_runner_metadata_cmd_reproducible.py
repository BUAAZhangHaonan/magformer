...
def test_lightdepth_stage_b_f5_magformer_dry_run_metadata_cmd_is_reproducible(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    script = repo_root / "scripts" / "experiments" / "run_0831_1k_20ep_1024_lightdepth_stage_b_f5_magformer.sh"
...
def test_lightdepth_stage_b_f5_magformer_supports_convnextlite_variants(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    script = repo_root / "scripts" / "experiments" / "run_0831_1k_20ep_1024_lightdepth_stage_b_f5_magformer.sh"
    dataset_root = tmp_path / "0831_1K"
    (dataset_root / "annotations").mkdir(parents=True)
    ...
    assert "model.magformer.modality_fusion.mode=prior_guided_cross_attn" in res.stdout
    assert "model.magformer.modality_fusion.priors=[edge,valid-hole,variance]" in res.stdout
