...
def test_pretrained_baselines_runner_dry_run_lists_three_models(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    script = repo_root / "scripts" / "experiments" / "run_0831_1k_20ep_1024_pretrained_baselines.sh"
    ...
    assert "START maskrcnn_pretrained_1024" in res.stdout
    assert "START official_mask2former_pretrained_1024" in res.stdout
    assert "START yolov8_seg_pretrained_1024" in res.stdout
