...
runner_log "${MODE}" "${RUN_LOG}" "[campaign-gpu0] wait_free_mb=${WAIT_FREE_MB} wait_sleep_sec=${WAIT_SLEEP_SEC}"
...
run_if_missing() {
  ...
  if [[ -f "${done_marker}" ]]; then
    runner_log "${MODE}" "${RUN_LOG}" "[campaign-gpu0] skip ${label} after wait: ${done_marker}"
    return 0
  fi
