...
case "${VARIANT}" in
  mobilenetv3_crossattn_edge)
    MODEL_ID="magformer_lightdepth_mobilenetv3_crossattn_edge"
    EXTRA_OVERRIDES+=(
  ...
  mobilenetv3_crossattn_edge_validhole)
    MODEL_ID="magformer_lightdepth_mobilenetv3_crossattn_edge_validhole"
    EXTRA_OVERRIDES+=(
      "model.magformer.modality_fusion.mode=cross_attn"
    ...
    ;;
  mobilenetv3_crossattn_edge_validhole_variance)
    MODEL_ID="magformer_lightdepth_mobilenetv3_crossattn_edge_validhole_variance"
    EXTRA_OVERRIDES+=(
      "model.magformer.modality_fusion.mode=cross_attn"
...
if run_train_once; then
  ...
  if rg -qi "outofmemoryerror|cuda out of memory" "${RUN_LOG}"; then
    runner_log "${MODE}" "${RUN_LOG}" "[0831-1k-20ep-1024-lightdepth-stage-b-magformer] OOM detected, retry with batch=2"
...
runner_log "${MODE}" "${RUN_LOG}" "[0831-1k-20ep-1024-lightdepth-stage-b-magformer] done"
