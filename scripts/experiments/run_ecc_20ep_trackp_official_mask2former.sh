...
# Canonicalize paths because the wrapper changes cwd to the Mask2Former repo root.
OUT="$(cd "${OUT}" && pwd)"
DATASET_ROOT="$(cd "${DATASET_ROOT}" && pwd)"
RUN_LOG="$(runner_setup_log "${OUT}" "${MODE}")"

runner_log "${MODE}" "${RUN_LOG}" "[official-mask2former-ecc-20ep-trackp] mode=${MODE}"
runner_log "${MODE}" "${RUN_LOG}" "[official-mask2former-ecc-20ep-trackp] smoke=${SMOKE}"
runner_log "${MODE}" "${RUN_LOG}" "[official-mask2former-ecc-20ep-trackp] register=${REGISTER}"
runner_log "${MODE}" "${RUN_LOG}" "[official-mask2former-ecc-20ep-trackp] run_tag=${RUN_TAG} candidate=${CANDIDATE_ID}"
runner_log "${MODE}" "${RUN_LOG}" "[official-mask2former-ecc-20ep-trackp] dataset_root=${DATASET_ROOT}"
runner_log "${MODE}" "${RUN_LOG}" "[official-mask2former-ecc-20ep-trackp] output_dir=${OUT}"
...
WEIGHTS_1CLASS="${REPO_ROOT}/output/pretrained/$(basename "${WEIGHTS_URL}" .pkl)_1class_v2.pth"
...
run_train_cmd() {
  ...
  local cmd="cd '${REPO_ROOT}' && conda run -n magformer python baselines/run_official_mask2former_ecc.py \
    --register '${REGISTER}' \
    --dataset-root '${DATASET_ROOT}' \
...
if run_train_cmd "${MAX_ITER}" "${SOLVER_STEPS}" "${WARMUP_ITERS}" "${IMS_PER_BATCH}" "${CHECKPOINT_PERIOD}" "${EVAL_PERIOD}"; then
  ...
  if rg -qi "outofmemoryerror|cuda out of memory" "${RUN_LOG}"; then
    runner_log "${MODE}" "${RUN_LOG}" "[official-mask2former-ecc-20ep-trackp] OOM detected, retry with batch=4 and epoch-aligned budget"
...
runner_log "${MODE}" "${RUN_LOG}" "[official-mask2former-ecc-20ep-trackp] done"
