...
: "${REPO_ROOT:?REPO_ROOT must be set before sourcing ecc_common.sh}"
DATASET_PARENT_ROOT="$(cd "${REPO_ROOT}/.." && pwd)"
...
ecc_default_dataset_root() {
  ...
  if [[ "${reg}" == "0831" ]]; then
    echo "${DATASET_PARENT_ROOT}/magformer_datasets/0831_1K"
  elif [[ "${reg}" == "0909" ]]; then
    echo "${DATASET_PARENT_ROOT}/magformer_datasets/0909_512_0.12K"
  elif [[ "${reg}" == "20260318_1k_1566" ]]; then
    echo "${DATASET_PARENT_ROOT}/magformer_datasets/20260318_1K_1566"
  else
    echo "${DATASET_PARENT_ROOT}/magformer_datasets/${raw}"
  fi
}
...
ecc_ensure_dataset_stats_manifest() {
  ...
  conda run -n magformer python3 "${REPO_ROOT}/scripts/analysis/ensure_dataset_stats.py" --dataset-root "${dataset_root}" --field manifest_path
}

ecc_rgb_stats_json_for_dataset_root() {
  local dataset_root="${1}"
  conda run -n magformer python3 "${REPO_ROOT}/scripts/analysis/ensure_dataset_stats.py" --dataset-root "${dataset_root}" --field rgb_stats_path
}

ecc_depth_stats_json_for_dataset_root() {
  local dataset_root="${1}"
  conda run -n magformer python3 "${REPO_ROOT}/scripts/analysis/ensure_dataset_stats.py" --dataset-root "${dataset_root}" --field depth_stats_path
}
