...
case "${VARIANT}" in
  mobilenetv3_priorguidedcrossattn_edge)
    MODEL_ID="magformer_lightdepth_mobilenetv3_priorguidedcrossattn_edge"
    EXTRA_OVERRIDES+=(
      "model.magformer.modality_fusion.mode=prior_guided_cross_attn"
  ...
  mobilenetv3_priorguidedcrossattn_edge_validhole)
    MODEL_ID="magformer_lightdepth_mobilenetv3_priorguidedcrossattn_edge_validhole"
    EXTRA_OVERRIDES+=(
      "model.magformer.modality_fusion.mode=prior_guided_cross_attn"
  ...
  mobilenetv3_priorguidedcrossattn_edge_validhole_variance)
    MODEL_ID="magformer_lightdepth_mobilenetv3_priorguidedcrossattn_edge_validhole_variance"
    EXTRA_OVERRIDES+=(
      "model.magformer.modality_fusion.mode=prior_guided_cross_attn"
  ...
  convnextlite_priorguidedcrossattn_edge)
    MODEL_ID="magformer_lightdepth_convnextlite_priorguidedcrossattn_edge"
  ...
  convnextlite_priorguidedcrossattn_edge_validhole)
    MODEL_ID="magformer_lightdepth_convnextlite_priorguidedcrossattn_edge_validhole"
    CFG_BASE="${REPO_ROOT}/configs/magformer_0831_1k_20ep_1024_lightdepth_convnextlite.yaml"
    EXTRA_OVERRIDES+=(
      "model.magformer.modality_fusion.mode=prior_guided_cross_attn"
      "model.magformer.modality_fusion.priors=[edge,valid-hole]"
  ...
  convnextlite_priorguidedcrossattn_edge_validhole_variance)
    MODEL_ID="magformer_lightdepth_convnextlite_priorguidedcrossattn_edge_validhole_variance"
    CFG_BASE="${REPO_ROOT}/configs/magformer_0831_1k_20ep_1024_lightdepth_convnextlite.yaml"
    EXTRA_OVERRIDES+=(
      "model.magformer.modality_fusion.mode=prior_guided_cross_attn"
      "model.magformer.modality_fusion.priors=[edge,valid-hole,variance]"
...
runner_exec "${MODE}" "${RUN_LOG}" "cd '${REPO_ROOT}' && conda run -n magformer python scripts/analysis/write_run_metadata.py \
  --phase start \
  --out-dir '${OUT}' \
  --track 0831_1k_20ep_1024_lightdepth_stage_b_f5 \
  --register '0831' \
  --dataset-root '${DATASET_ROOT}' \
  --model-id '${MODEL_ID}' \
...
if run_train_once; then
  ...
  if rg -qi "outofmemoryerror|cuda out of memory" "${RUN_LOG}"; then
    runner_log "${MODE}" "${RUN_LOG}" "[0831-1k-20ep-1024-lightdepth-stage-b-f5-magformer] OOM detected, retry with batch=2"
...
runner_log "${MODE}" "${RUN_LOG}" "[0831-1k-20ep-1024-lightdepth-stage-b-f5-magformer] done"
