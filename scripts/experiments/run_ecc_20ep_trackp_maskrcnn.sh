...
runner_log "${MODE}" "${RUN_LOG}" "[maskrcnn-ecc-20ep-trackp] smoke=${SMOKE}"
runner_log "${MODE}" "${RUN_LOG}" "[maskrcnn-ecc-20ep-trackp] register=${REGISTER}"
runner_log "${MODE}" "${RUN_LOG}" "[maskrcnn-ecc-20ep-trackp] run_tag=${RUN_TAG} candidate=${CANDIDATE_ID}"
runner_log "${MODE}" "${RUN_LOG}" "[maskrcnn-ecc-20ep-trackp] dataset_root=${DATASET_ROOT}"
runner_log "${MODE}" "${RUN_LOG}" "[maskrcnn-ecc-20ep-trackp] output_dir=${OUT}"
...
if run_train_cmd "${MAX_ITER}" "${SOLVER_STEPS}" "${WARMUP_ITERS}" "${IMS_PER_BATCH}" "${CHECKPOINT_PERIOD}" "${EVAL_PERIOD}"; then
  ...
  if rg -qi "outofmemoryerror|cuda out of memory" "${RUN_LOG}"; then
    runner_log "${MODE}" "${RUN_LOG}" "[maskrcnn-ecc-20ep-trackp] OOM detected, retry with batch=4 and epoch-aligned budget"
...
EON
    ...
    runner_log "${MODE}" "${RUN_LOG}" "[maskrcnn-ecc-20ep-trackp] NaN/Inf divergence detected, retry with lower lr=${retry_lr}"
...
runner_log "${MODE}" "${RUN_LOG}" "[maskrcnn-ecc-20ep-trackp] done"
