...
runner_log "${MODE}" "${RUN_LOG}" "[msmformer-ecc-20ep-tracks] output_dir=${OUT}"
PRETRAINED_ARG=""
if [[ -f "${LOCAL_PRETRAINED}" ]]; then
  PRETRAINED_ARG="--pretrained '${LOCAL_PRETRAINED}'"
  runner_log "${MODE}" "${RUN_LOG}" "[msmformer-ecc-20ep-tracks] pretrained=${LOCAL_PRETRAINED}"
else
  runner_log "${MODE}" "${RUN_LOG}" "[msmformer-ecc-20ep-tracks] pretrained=none"
fi
...
if run_train_cmd "${MAX_ITER}" "${SOLVER_STEPS}" "${WARMUP_ITERS}" "${IMS_PER_BATCH}" "${CHECKPOINT_PERIOD}" "${EVAL_PERIOD}"; then
  ...
  if rg -qi "outofmemoryerror|cuda out of memory" "${RUN_LOG}"; then
    runner_log "${MODE}" "${RUN_LOG}" "[msmformer-ecc-20ep-tracks] OOM detected, retry with batch=4 and epoch-aligned budget"
...
runner_log "${MODE}" "${RUN_LOG}" "[msmformer-ecc-20ep-tracks] done"
