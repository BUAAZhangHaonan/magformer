...
runner_log "${MODE}" "${RUN_LOG}" "[maskrcnn-0831-1k-20ep-scratch] mode=${MODE}"
...
run_train_cmd() {
  ...
  local cmd="cd '${REPO_ROOT}' && ${HF_ENV_PREFIX}conda run -n magformer python baselines/run_detectron2_ecc.py \
    --register '${REGISTER_RAW}' \
    --dataset-root '${DATASET_ROOT}' \
...
if run_train_cmd "${MAX_ITER}" "${SOLVER_STEPS}" "${WARMUP_ITERS}" "${IMS_PER_BATCH}" "${CHECKPOINT_PERIOD}" "${EVAL_PERIOD}"; then
  ...
  if rg -qi "outofmemoryerror|cuda out of memory" "${RUN_LOG}"; then
    runner_log "${MODE}" "${RUN_LOG}" "[maskrcnn-0831-1k-20ep-scratch] OOM detected, retry with batch=4 and epoch-aligned budget"
    fallback_iter=4440
...
runner_log "${MODE}" "${RUN_LOG}" "[maskrcnn-0831-1k-20ep-scratch] done"
