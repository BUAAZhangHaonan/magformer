...
runner_log "${MODE}" "${RUN_LOG}" "[yolov8-seg-ecc-20ep-trackp] mode=${MODE}"
runner_log "${MODE}" "${RUN_LOG}" "[yolov8-seg-ecc-20ep-trackp] smoke=${SMOKE}"
runner_log "${MODE}" "${RUN_LOG}" "[yolov8-seg-ecc-20ep-trackp] register=${REGISTER}"
...
if run_train_cmd "${BATCH}"; then
  ...
  if rg -qi "outofmemoryerror|cuda out of memory" "${RUN_LOG}"; then
    runner_log "${MODE}" "${RUN_LOG}" "[yolov8-seg-ecc-20ep-trackp] OOM detected, retry with batch=4 (same epochs)"
...
runner_log "${MODE}" "${RUN_LOG}" "[yolov8-seg-ecc-20ep-trackp] done"
