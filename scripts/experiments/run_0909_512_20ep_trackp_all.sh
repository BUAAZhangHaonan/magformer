...
OUTPUT_ROOT_DEFAULT_RUN="${REPO_ROOT}/output/experiments/0909_512_20ep_trackp"
OUTPUT_ROOT_DEFAULT_SMOKE="${REPO_ROOT}/output/experiments/0909_512_20ep_trackp_smoke"
...
runner_log "${MODE}" "${RUN_ALL_LOG}" "[0909-512-20ep-trackp-all] mode=${MODE}"
runner_log "${MODE}" "${RUN_ALL_LOG}" "[0909-512-20ep-trackp-all] smoke=${SMOKE}"
runner_log "${MODE}" "${RUN_ALL_LOG}" "[0909-512-20ep-trackp-all] dataset_root=${DATASET_ROOT}"
runner_log "${MODE}" "${RUN_ALL_LOG}" "[0909-512-20ep-trackp-all] output_root=${OUTPUT_ROOT}"
...
run_model_once() {
  ...
  if [[ -f "${done_marker}" ]]; then
    runner_log "${MODE}" "${RUN_ALL_LOG}" "[0909-512-20ep-trackp-all] SKIP ${model_id} ${run_tag} ${candidate} (already done)"
    return 0
  fi

  runner_log "${MODE}" "${RUN_ALL_LOG}" "[0909-512-20ep-trackp-all] START ${model_id} tag=${run_tag} candidate=${candidate}"
  ...
  runner_log "${MODE}" "${RUN_ALL_LOG}" "[0909-512-20ep-trackp-all] END ${model_id} tag=${run_tag} candidate=${candidate}"
}
...
if [[ "${SMOKE}" == "1" ]]; then
  ...
  run_summary
  runner_log "${MODE}" "${RUN_ALL_LOG}" "[0909-512-20ep-trackp-all] done(smoke)"
  exit 0
fi
...
for item in "${MODELS[@]}"; do
  ...
  runner_log "${MODE}" "${RUN_ALL_LOG}" "[0909-512-20ep-trackp-all] BEST ${model_id}=${best_candidate}"
...
runner_log "${MODE}" "${RUN_ALL_LOG}" "[0909-512-20ep-trackp-all] done"
