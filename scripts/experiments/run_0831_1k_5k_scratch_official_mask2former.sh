...
if not ckpts:
    print(f"[official-mask2former] params: no detectron2 checkpoint under {out_dir}", file=sys.stderr)
    raise SystemExit(0)
ckpt = torch.load(ckpts[-1], map_location="cpu")
state = ckpt.get("model", ckpt) if isinstance(ckpt, dict) else ckpt
if not isinstance(state, dict):
    print(
        f"[official-mask2former] params: unexpected checkpoint type: {type(ckpt)}",
        file=sys.stderr,
    )
    raise SystemExit(0)
n = sum(int(v.numel()) for v in state.values() if hasattr(v, "numel"))
(out_dir / "params_trainable.txt").write_text(str(n) + "\n", encoding="utf-8")
print("[official-mask2former] params_trainable:", n, "from", ckpts[-1].name)
PY
fi
