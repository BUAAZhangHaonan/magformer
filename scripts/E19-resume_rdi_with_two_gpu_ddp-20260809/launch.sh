...
launch() {
    ...
    local config=$4
    local output_dir=$5
    local log_file="$output_dir/console.log"
    ...
    tmux new-session -d -s "$session" bash -lc "
        ...
        tools/train.py --config '$config' 2>&1 | tee '$log_file'
    "
}

launch magformer-e19-rdi-lite-ddp 0,1 29619 \
    "$script_root/rdi_lite.yaml" \
    "$repo_root/output/experiments/E19-resume_rdi_with_two_gpu_ddp-20260809/rdi_lite"
launch magformer-e19-rdi-mbv3l-ddp 2,3 29620 \
