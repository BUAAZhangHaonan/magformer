...
class DepthPriorExtractor(nn.Module):
    ...
    def _compute_var(self, x: torch.Tensor) -> torch.Tensor:
        ...
        k = self.k
        pad = k // 2
        mu = F.avg_pool2d(x, kernel_size=k, stride=1, padding=pad, count_include_pad=False)
        var = F.avg_pool2d(x**2, kernel_size=k, stride=1, padding=pad, count_include_pad=False) - mu**2
    ...
    def forward(
        ...
        if self.use_rgb_edge and rgb is not None:
            ...
            priors["edge_consistency"] = (1.0 - (g_rgb - g_dep).abs()).clamp(0, 1)
        else:
...
class ConfidencePredictor(nn.Module):
    ...
    def __init__(
        ...
        assert len(feature_dims) == len(scale_keys), "Feature dimensions and scale keys must match"
...
class ModalityFusionModule(nn.Module):
    ...
    def __init__(
        ...
        if self.post_fuse_norm:
            self.post_norm = nn.ModuleList([nn.GroupNorm(8, ch) for ch in self.feature_dims])
        else:
            self.post_norm = None
        ...
        if self.training and m_maps:
            ...
            if self.noise_mask_weight > 0 and depth_noise_mask is not None:
                ...
                for key, m in m_maps.items():
                    ...
                    target_noise_mask = F.max_pool2d(target_noise_mask, kernel_size=3, stride=1, padding=1)
                    bce = F.binary_cross_entropy_with_logits(logits, 1.0 - target_noise_mask)
                    bces.append(bce)
    ...
    def _prepare_priors_ms(
        ...
        if compute_on == "full":
            ...
            compute_res_depth = F.interpolate(depth_raw, (h0, w0), mode="bilinear", align_corners=False)
            compute_res_rgb = (
                F.interpolate(rgb_image, (h0, w0), mode="bilinear", align_corners=False)
        ...
        for prior_name, prior_tensor in all_priors_single_res.items():
            for key, (h, w) in target_sizes.items():
                ...
                resized = F.interpolate(prior_tensor_4d, (h, w), mode="bilinear", align_corners=False)
