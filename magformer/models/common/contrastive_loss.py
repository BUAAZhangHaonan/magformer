...
class EQOContrastiveLoss(nn.Module):
    ...
    def forward(self, query_embeddings, targets, indices):
        ...
        for img_idx in range(batch_size):
            ...
            # Hungarian matching is 1:1, so "same GT object" never yields positive
            # pairs besides self. Instead, treat all other matched queries as
            # positives (foreground cluster) and unmatched queries as negatives
            # (background). This encourages query specialization: matched queries
            # form a coherent region in embedding space, separate from unmatched.
