...
# Import the optional build artifact without changing CUDA execution semantics.
MSDA = None
...
class MSDeformAttnFunction(Function):
    ...
    def forward(ctx, value, value_spatial_shapes, value_level_start_index, sampling_locations, attention_weights, im2col_step):
        if not value.is_cuda:
            raise RuntimeError(
                "MSDeformAttnFunction is CUDA-only; use "
                "ms_deform_attn_core_pytorch for CPU tensors"
            )
        if not _cuda_available:
            raise RuntimeError(
                "MSDeformAttn CUDA extension is unavailable: "
    ...
    def backward(ctx, grad_output):
        if not _cuda_available:
            raise RuntimeError(
                "MSDeformAttn CUDA extension became unavailable during backward"
            )
        ...
        grad_value = grad_value.view(value.shape)
