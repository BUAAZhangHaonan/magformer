...
def build_model(config: MagFormerConfig) -> nn.Module:
    ...
    if arch == "MagFormer":
        small_object_sample_threshold = int(
            config.model.small_object_sample_threshold)
        if small_object_sample_threshold != 0:
            raise ValueError(
                "model.small_object_sample_threshold is a retired, unsupported "
                "criterion sampling heuristic; it is not a data-augmentation "
                "setting and was previously accepted without being wired. "
                "Remove it or set model.small_object_sample_threshold=0 "
                f"(got {small_object_sample_threshold})."
            )
