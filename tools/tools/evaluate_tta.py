...
from tools.soft_nms import soft_nms as soft_nms_fn
...
def parse_args():
    ...
    # Soft-NMS options
    parser.add_argument("--nms-method", type=str, default="mask_cluster",
                        choices=["mask_cluster", "soft_gaussian", "soft_linear", "soft_hard"],
                        help="NMS method: mask_cluster (original soft mask clustering), "
                             "soft_gaussian/soft_linear/soft_hard (soft-NMS variants). "
                             "soft_hard is standard hard NMS via soft-NMS code path.")
    parser.add_argument("--nms-sigma", type=float, default=0.5,
                        help="Gaussian decay sigma for soft-NMS. Lower = more aggressive decay.")
    parser.add_argument("--soft-nms-iou", type=float, default=0.3,
                        help="IoU threshold for soft-NMS (lower than hard NMS since soft-NMS is more forgiving). "
                             "Also used as bbox prefilter threshold inside soft-NMS.")
    ...
    parser.add_argument("--export-score-thresh", type=float, default=0.0,
                        help="Prediction score threshold for COCO export (default: 0.0).")
...
def tta_inference_single_image(model, images, depths, scales, hflip, device,
                                ...
                                nms_method="mask_cluster", nms_sigma=0.5, soft_nms_iou=0.3):
    ...
    if merge_method == "wbf":
        ...
        # Concatenate all augmentation predictions
        ...
        if nms_method == "mask_cluster":
            # Original soft mask clustering (default, backward compatible)
            return nms_merge(
                all_scores,
                all_masks,
                all_cats,
            ...
            # Soft-NMS path: gaussian, linear, or hard
            method_map = {
                "soft_gaussian": "gaussian",
                "soft_linear": "linear",
                "soft_hard": "hard",
            }
            snms_method = method_map[nms_method]
            return soft_nms_fn(
                all_scores,
                all_masks,
                all_cats,
...
def main():
    ...
    print(f"[TTA] NMS method: {args.nms_method}")
    if args.nms_method != "mask_cluster":
        print(f"[TTA] Soft-NMS sigma: {args.nms_sigma}, IoU threshold: {args.soft_nms_iou}")
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        for ens_model in ensemble_models:
            ens_pred = tta_inference_single_image(
                ...
                merge_method=args.merge_method, wbf_iou=args.wbf_iou, wbf_skip_thr=args.wbf_skip_thr,
                wbf_conf_type=args.wbf_conf_type, wbf_overflow=args.wbf_overflow,
                nms_method=args.nms_method, nms_sigma=args.nms_sigma, soft_nms_iou=args.soft_nms_iou,
            )
    ...
    elif args.nms_method != "mask_cluster":
        results_filename = f"tta_{args.nms_method}_sigma{args.nms_sigma}_iou{args.soft_nms_iou}_coco_results.json"
    else:
