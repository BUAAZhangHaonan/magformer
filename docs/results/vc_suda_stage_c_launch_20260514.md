...
- The currently running Stage C job was launched before the pseudo-label observability patch, so its existing `metrics_log.jsonl` rows do not contain `train/pseudo_kept_count`, `train/pseudo_empty_images`, `train/pseudo_threshold`, or `train/pseudo_keep_rate`. Future resume/new runs from the patched code will write those fields on Stage C train log rows.
