...
- [VC-SUDA Stage C R10 no-depth-noise continuation plan, 2026-05-15](vc_suda_stage_c_r10_no_depth_noise_plan_20260515.md): continue from R8B `ckpt999`, keep R8B settings, and set only `data.depth_noise.gaussian_std=0.0`. The run keeps all checkpoints with `runtime.checkpoint_max_keep: null`. The `ckpt249` external 1024 backmap gate must beat the R8B `ckpt999` target_unlabeled200 segm AP `0.319162` and should be competitive with the current global best R8B `ckpt749` segm AP `0.319922`.
