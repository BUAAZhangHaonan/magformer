...
- [VC-SUDA R42 point65536 original-split smoke, 2026-05-16](vc_suda_r42_point65536_original_split_plan_20260516.md): prepared the single-variable training-side mask point sampling check from the R35/R12 original-split true-resume baseline. The only formal training variable is `model.magformer.mask_former.train_num_points: 12544 -> 65536`; original split, R12 `ckpt499` true-resume, `image_size=1024`, `max_iter=750`, `ims_per_batch=4`, LR, unsup weight, pseudo threshold, `num_object_queries=200`, `oversample_ratio=3.0`, and `importance_sample_ratio=0.75` stay fixed. R41 eval1536 failed, so R42 stops the inference-resolution route and tests whether denser training mask points can address the R38/R40 mask-set coverage/localization bottleneck. The tmux smoke on GPUs `4,5,6,7` with CUDA MSDA passed from iter `499` to `500`, exit code `0`, finite losses, no Traceback/OOM/non-finite/missing/unexpected/shape-mismatch errors, peak sampled GPU memory `21459/21219/21219/21399 MiB`, peak sampled RAM about `61.0%`, and elapsed time `202s`. Formal 250-iter gate was not started.
