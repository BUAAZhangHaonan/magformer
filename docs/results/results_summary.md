...
- [VC-SUDA Stage C R32 plus25 unsup0 plan, 2026-05-16](vc_suda_stage_c_r32_plus25_unsup0_plan_20260516.md): created the R32 single-variable plan and config from R31. R32 keeps the R31 +25/-25 split, 1024 path, 32K source, R12 `ckpt499` warm start, LR, pseudo threshold, pseudo `max_instances`, loss weights, augmentation, `max_iter=250`, checkpoint/eval periods, and runtime settings unchanged, while setting only `vc_suda.unsupervised_weight: 0.0`. The gate is formal external `target_unlabeled200` 1024 backmap bbox+segm: continue only if segm AP is `>=0.320048`, record but do not extend for `0.319162-0.320048`, and stop below `0.319162`.
