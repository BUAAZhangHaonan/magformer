...
- [VC-SUDA Stage C R34 plus25 true-resume plan, 2026-05-16](vc_suda_stage_c_r34_plus25_true_resume_plan_20260516.md): prepared the single-variable R34 config from R31 by replacing model-only `finetune_weights` warm-start with true `runtime.resume` from R12 `checkpoint_iter_0000499.pth`, setting `model.finetune_weights: null` and `solver.max_iter: 750`. R34 keeps the R31 +25/-25 split, 1024 route, 32K source, LR/losses/pseudo threshold, `unsupervised_weight=0.02`, `max_instances`, and augmentation unchanged. The first gate is external `target_unlabeled200` 1024 backmap bbox+segm after the 250-iter continuation: continue only if segm AP `>=0.320048`; record but do not extend for `0.319162-0.320048`; stop below `0.319162`.
