...
- [VC-SUDA Stage C R35 original split true-resume plan, 2026-05-16](vc_suda_stage_c_r35_original_split_true_resume_plan_20260516.md): prepared a single-variable true-resume isolation run from R12 `checkpoint_iter_0000499.pth` to iter `750`, keeping the original R12 target split `target_labeled=25` / `target_unlabeled=200`, 1024 path, 32K source, LR/loss/pseudo settings, and augmentation unchanged. `model.finetune_weights` is disabled and `runtime.resume` points to the R12 `ckpt499`; checkpoint/eval periods are `250` so iter `750` can produce the gate checkpoint/eval signal. Gate: external `target_unlabeled200` 1024 backmap bbox+segm; segm AP `>=0.320048` means original-split continuation does not hurt and `+25` data mixing is the suspect, `0.319162-0.320048` means record and do not extend, and `<0.319162` means true-resume continuation itself also hurts.
