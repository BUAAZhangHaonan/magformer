...
### 纯采样打捞臂 @4K（2026-09-23 00:55 判读）— 灾难级失败，bass 全形态死透，战役终止

| 指标 | 打捞@4K | 锚点@4K | 差 |
|---|---|---|---|
| segm AP | 0.7157 | 0.8704 | **−15.5pt**（比全形态 0.7510 更差） |
| segm APs | 0.0353 | 0.2692 | **−23.4pt** |
| segm AP95 | 0.1853 | 0.4339 | −24.9pt（边界质量摧毁） |
| bbox AP | 0.1301 | 0.8566 | **−72.6pt** |

- **毒性主犯改判**：纯采样（软标签关/λ=1/带Dice=0）比全形态更毒 ⇒ 软标签实际在
  缓冲边界 0/1 冲突；真毒 = 采样重分布 + balanced_ce 带内豁免（边界外点 100× 降权
  被豁免 = 边界双向梯度战争放大 100×）。
- **终审结论：P3（bass 两种形态）+ P4（seed）训练侧方案全部出局。** 两提案收敛
  战役终止。微调体制下（锚点=seal 持平）无任何方案产生正增量；噪声带先验
  （EVIDENCE §7）与所有门臂读数一致。
- 处置（用户指令 2026-09-23）：停止优化战役 → 深度清理 → 封档。
  DPE 验证 = 历史消融记录 dpe_off 0.8510 vs c0_corrected 0.8603（+0.9pt，轻度有效，
  保留于最终版）；concat 对照 = M2F concat 头 0.91 mAP / 0.354 APs > 本族 0.87/0.27。
